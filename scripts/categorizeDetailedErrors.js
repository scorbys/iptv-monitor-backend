...
// Client-side categorization with updated keywords
...
async function categorizeDetailedNotifications() {
  console.log('=== Categorizing Notifications with Detailed Error Messages ===\n');
  ...
  try {
    ...
    // Get notifications with error details (containing "-")
    const notifications = await db.collection('notifications').find({
      message: { $regex: /\s-\s/ }, // Contains " - " which indicates error details
      errorCategory: { $in: [null, '', 'External'] }
    }).limit(50).toArray();

    console.log(`Found ${notifications.length} notifications with detailed errors\n`);
    ...
    for (let i = 0; i < notifications.length; i++) {
      ...
      try {
        ...
        if (category) {
          ...
          // Store first example for each category
          if (!examples[category]) {
            examples[category] = notification.message;
          }
    ...
    Object.entries(categoryCount)
      ...
      .forEach(([cat, count]) => {
        ...
        console.log(`    Example: ${examples[cat].substring(0, 60)}...`);
      });

    console.log('\n✅ Detailed error messages are now categorized!');
    console.log('You can now see more variety in NotifPage and ML Dashboard');
...
categorizeDetailedNotifications()
  .then(() => {
    console.log('\n=== Script Complete ===');
    process.exit(0);
  })
